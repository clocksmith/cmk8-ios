...
- (void)drawPartGroups:(NSArray *)partGroups {
  ...
  if (!_upwardTriangle) {
    _upwardTriangle = [[UIImageView alloc] initWithImage:
                       [UIImage imageNamed:@"upward_triangle_dark_gray"]];

    _upwardTriangle.frame = CGRectMake(0, 0, frameWidth, _PAGER_ARROW_HEIGHT);
    _upwardTriangle.bounds = CGRectInset(_upwardTriangle.frame,
                                         _PAGER_ARROW_PADDING,
                                         _PAGER_ARROW_PADDING);
  ...
  if (!_scrollView) {
    ...
    _scrollView.frame = CGRectMake(0,
                                   _PAGER_ARROW_HEIGHT,
                                   frameWidth,
                                   frameHeight - 2 * _PAGER_ARROW_HEIGHT);
  ...
  if (!_downwardTriangle) {
    UIImageView *downwardTriangle = [[UIImageView alloc] initWithImage:
                                     [UIImage imageNamed:@"downward_triangle_dark_gray"]];
    downwardTriangle.frame = CGRectMake(0,
                                        frameHeight - _PAGER_ARROW_HEIGHT,
                                        frameWidth,
                                        _PAGER_ARROW_HEIGHT);
    downwardTriangle.bounds = CGRectInset(downwardTriangle.frame,
                                          _PAGER_ARROW_PADDING,
                                          _PAGER_ARROW_PADDING);
  ...
  for (CMKPartGroupModel *partGroup in _partGroups) {
    CGRect partGroupFrame = CGRectMake(0,
                                       partGroupIndex * scrollViewHeight,
                                       scrollViewWidth,
                                       scrollViewHeight);
...
- (void)selectPartGroup:(CMKPartGroupModel *)partGroup isAnimated:(BOOL)animated {
  ...
  [_scrollView setContentOffset:CGPointMake(0, pageIndex * _scrollView.frame.size.height)
                       animated:YES];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
  ...
  if (strongDelegate &&
      [strongDelegate respondsToSelector:@selector(partGroupChooserView:didSelectPartGroup:)]) {
