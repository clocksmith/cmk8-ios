...
- (instancetype)initWithFrame:(CGRect)frame
  ...
  if (self) {
    ...
    int partsCount = [partGroup.parts count];
    ...
    CGRect topRowFrame = CGRectMake(0,
                                    PART_GROUP_VIEW_TITLE_HEIGHT +
                                    (partsCount < _MIN_PARTS_FOR_DOUBLE_ROW ? bothRowsHeight / 4 : 0),
                                    frameWidth,
                                    bothRowsHeight / 2);
    UIView *topRow = [[UIView alloc] initWithFrame:topRowFrame];
    [self addSubview:topRow];

    if (partsCount < _MIN_PARTS_FOR_DOUBLE_ROW) {
      [self fillRow:topRow withParts:partGroup.parts];
    } else {
      int topRowCount = (int)ceil((float)partsCount / 2);
      int bottomRowCount = partsCount - topRowCount;
      ...
      CGRect bottomRowFrame = CGRectMake(0,
                                         PART_GROUP_VIEW_TITLE_HEIGHT + bothRowsHeight / 2,
                                         frameWidth,
                                         bothRowsHeight / 2);
...
- (void)fillRow:(UIView *)row withParts:(NSArray *)parts {
  ...
  for (CMKPartModel *part in parts) {
    ...
    CGRect partImageFrame =
      CGRectMake(partIndex * deltaX + (partsCount == _MAX_PARTS_PER_ROW ?
                                       0 :
                                       (deltaX / [parts count] / 2)),
                 0,
                 partImageFrameWidth,
                 partImageFrameHeight);
