...
- (instancetype)init {
  NSLog(@"%@ init ", _TAG);
  self = [super initWithTitle:COMPARE_TITLE_STRING withImageName:COMPARE_ICON_IMAGE withScreenName:COMPARE_SCREEN];
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  NSArray *allStarredKartConfigurations = [CMKStarredKartConfigurationUtils allStarredKartConfigurations];
  ...
  if ([allStarredKartConfigurations count] < 2) {
    ...
    if (!_placeholderView) {
      _placeholderView = [[[NSBundle mainBundle] loadNibNamed:NSStringFromClass([CMKComparePlaceholderView class])
                                                        owner:self
    ...
    [self setupSpinnerView:self.leftSpinnerView withAllStarredKartConfigurations:allStarredKartConfigurations];
    [self setupSpinnerView:self.rightSpinnerView withAllStarredKartConfigurations:allStarredKartConfigurations];
...
- (void)spinner:(CMKSpinnerView *)spinner didSelectItem:(id<CMKSpinnerItem>)item {
  ...
  if (spinner == self.leftSpinnerView) {
    ...
    [CMKAnalyticsUtils sendCompareViewSelectedEvent:self.tracker withKartConfiguration:_model.leftKartConfiguration];
    ...
    [CMKAnalyticsUtils sendCompareViewSelectedEvent:self.tracker withKartConfiguration:_model.rightKartConfiguration];
  } else {
...
- (void)partImageView:(UIImageView *)partImageView didTapPart:(CMKPartModel *)part {
  ...
  [CMKAnalyticsUtils sendPartViewClickedEvent:self.tracker withScreenName:COMPARE_SCREEN withPart:part];
...
- (void)updateKartConfigurationViews {
  [self.leftKartConfigurationView update:_model.leftKartConfiguration isAnimated:_animateViews || _isFirstLoad];
  [self.rightKartConfigurationView update:_model.rightKartConfiguration isAnimated:_animateViews || _isFirstLoad];
}

- (void)updateStatsView {
  [self.statsView updateWithLeftStats:_model.leftKartConfiguration.kartStats
...
- (void)setupSpinnerView:(CMKSpinnerView *)spinnerView
  ...
  spinnerView.selectedItemlabel.font =
      [UIFont fontWithName:HELVETICA_NEUE_FONT
                      size:IS_IPAD ? COMPARE_SPINNER_FONT_SIZE_IPAD : COMPARE_SPINNER_FONT_SIZE_IPHONE];
